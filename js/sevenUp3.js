...
metro.putData = function(list){
	(function setBig(){  //大格子初始化设置
		...
		while(i < bigBox.length){
			...
			//计算div.bigBox>div.innerBox的宽度，向上取整，设定div.bigBox的合适宽度
...
function createTestData(n){
	...
	for(var i = 0; i < n; i++) {
		...
		if(!(Math.random()*3 >> 0)){ //输出大<a href='http://www.zztuku.com/tags/模块.html' target='_blank'><u>模块</u></a>测试数据。宽高在一个范围内都是随意的，未做其他限制
			height = Math.floor(Math.random()*200 + 100);
			width = Math.floor(Math.random()*200 + 100);
			content += '<div class="MBox"><div class="widgetBox bigBox" style="background:'+color+'"><div style="width:' + width +'px;height:' + height +'px;margin:0 auto;" class="innerBox"></div></div></div>';
		}else{ //输出普通<a href='http://www.zztuku.com/tags/模块.html' target='_blank'><u>模块</u></a>数据
